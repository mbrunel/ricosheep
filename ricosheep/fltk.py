...
class CustomCanvas:
    ...
    def __init__(self, width, height, center, refresh_rate, events=None):
        ...
        # prevent window from being resized since fltk doesn't handle it
        self.root.resizable(False, False)

        # if requested center the window
        if center:
            position_right = int(self.root.winfo_screenwidth() / 2 - width / 2)
            position_down = int(self.root.winfo_screenheight() / 3 - height / 2)
...
def cree_fenetre(largeur, hauteur, center=True, frequence=100):
    """
    ...
    __canevas = CustomCanvas(largeur, hauteur, center, frequence)
...
def set_cursor(cursor):
    ...
    """
    __canevas.root.config(cursor=cursor)
...
def image(x, y, fichier, width=0, height=0, ancrage="center", tag=""):
    """
    ...
    if PIL_AVAILABLE:
        ...
        if width and height:
            iw, ih = img.size
            if (width < iw and iw - width > ih - height) or (width >= iw and width - iw < height - ih):
                coeff = width / iw
            else:
                coeff = height / ih
            img = img.resize((int(iw * coeff), int(ih * coeff)), Image.ANTIALIAS)
